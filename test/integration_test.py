...
def test_patch_augmentation_1():
    ...
    background_image = loader.load_image(SAMPLE_PATCHMENTATION_BACKGROUND_IMAGE_1)
    
    patches = dataset.imagePatches[0].patches + dataset.imagePatches[1].patches + dataset.imagePatches[2].patches
...
def test_patch_augmentation_2():
    dataset = loader.load_yolo_dataset(SAMPLE_PATCHMENTATION_FOLDER_IMAGES, SAMPLE_PATCHMENTATION_FOLDER_ANNOTATIONS, SAMPLE_PATCHMENTATION_FILE_NAMES)
    background_image = loader.load_image(SAMPLE_PATCHMENTATION_BACKGROUND_IMAGE_2)
    
    patches = dataset.imagePatches[0].patches
    imagePatch = patch_augmentation(patches, background_image)

    patches = dataset.imagePatches[1].patches
    imagePatch = patch_augmentation(patches, imagePatch)

    patches = dataset.imagePatches[2].patches
    imagePatch = patch_augmentation(patches, background_image)
